...
To use external instead of inline styles, switch the [markup.highlight.noClasses option](https://gohugo.io/functions/highlight/#options) and the params.noClasses option to false in the [site configuration](https://gohugo.io/getting-started/configuration/). The external style will only be included on pages with syntax highlighting.

The default highlighting style is called [friendly](https://xyproto.github.io/splash/docs/friendly.html).
To change the highlighting style, generate a style sheet with the [hugo gen chromastyles](https://gohugo.io/commands/hugo_gen_chromastyles/) command and place it under `assets/css/syntax.css`.
